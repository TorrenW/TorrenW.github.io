...
function printSpellBack($spell) {
    ...
    if (strpos($spell['components'], 'М') !== false) {
        ...
        if (preg_match('/М\s*\((.*?)\)/', $spell['components'], $matches)) {
            ...
            echo "<div style='height: -webkit-fill-available; display: contents;'><p class='material-component' style='font-family:MartaBold;'>{$mComponents}</p><div>";
        }
    }
    echo "</td></tr></table>";
...
function printSpellList($spells) {
    ...
    foreach ($spells as $spell) {
        $spellCardHtml = "<div class='spell-card'>" . htmlspecialchars(printSpellCardToString($spell)) . "</div>";
        $spellBackHtml = "<div class='spell-back-block'>" . htmlspecialchars(printSpellBackToString($spell)) . "</div>";
        $spellData = json_encode(['card' => $spellCardHtml, 'back' => $spellBackHtml]);
